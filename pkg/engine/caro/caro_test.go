...
func TestSnapshot(t *testing.T) {
	...
	b := NewBoard(3)
	if err := b.Apply(Position{X: 0, Y: 0}, 'X'); err != nil {
		t.Fatalf("unexpected setup error: %v", err)
	}
	if err := b.Apply(Position{X: 2, Y: 1}, 'O'); err != nil {
		t.Fatalf("unexpected setup error: %v", err)
	}
...
func TestNewGame(t *testing.T) {
	...
	if g.board.Size != 10 {
		t.Fatalf("expected board size 10, got %d", g.board.Size)
	}
